...
namespace Hoehoe
{
    ...
    public partial class Thumbnail
    {
        ...
        {
            var mc = Regex.Match(string.IsNullOrEmpty(args.Extended) ? args.Url : args.Extended, "^http://moby\\.to/(\\w+)$", RegexOptions.IgnoreCase);
            if (!mc.Success)
            {
                return false;
            }

            args.AddThumbnailUrl(args.Url, mc.Result("http://mobypicture.com/?${1}:small"));
            return true;
        }
