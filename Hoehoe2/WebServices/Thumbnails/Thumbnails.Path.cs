...
namespace Hoehoe
{
    ...
    public partial class Thumbnail
    {
        ...
        private static bool Path_GetUrl(GetUrlArgs args)
        {
            var mc = Regex.Match(string.IsNullOrEmpty(args.Extended) ? args.Url : args.Extended, @"^https?://path\.com/p/(\w+)$", RegexOptions.IgnoreCase);
            if (!mc.Success)
            {
                return false;
            }
