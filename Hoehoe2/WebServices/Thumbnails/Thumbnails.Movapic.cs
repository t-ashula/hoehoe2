...
namespace Hoehoe
{
    public partial class Thumbnail
    {
        ...
        {
            var mc = Regex.Match(string.IsNullOrEmpty(args.Extended) ? args.Url : args.Extended, "^http://movapic\\.com/pic/(\\w+)$", RegexOptions.IgnoreCase);
            if (!mc.Success)
            {
                return false;
            }

            args.AddThumbnailUrl(args.Url, mc.Result("http://image.movapic.com/pic/s_${1}.jpeg"));
            return true;
        }
