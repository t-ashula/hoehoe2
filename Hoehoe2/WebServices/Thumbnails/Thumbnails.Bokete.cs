...
namespace Hoehoe
{
    public partial class Thumbnail
    {
        ...
        private static bool Bokete_GetUrl(GetUrlArgs args)
        {
            var mc = Regex.Match(string.IsNullOrEmpty(args.Extended) ? args.Url : args.Extended, "^http://bokete\\.jp/boke/(\\d+)$", RegexOptions.IgnoreCase);
            if (!mc.Success)
            {
                return false;
            }

            args.AddThumbnailUrl(args.Url, mc.Result("http://ss.bokete.jp/${1}.jpg"));
            return true;
        }
